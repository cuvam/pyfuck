...
3. Using the fact that `[]==[]` = True (True+True=2, so it spiritually equals 1) in Python, construct an equation using just 1's and `+`/`*`, then replace 1 with `[]==[]`, for each character
